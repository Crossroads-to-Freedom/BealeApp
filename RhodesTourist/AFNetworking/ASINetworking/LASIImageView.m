...
- (void)downloadImage
{
    ...
    if ([[ASIDownloadCache sharedCache] isCachedDataCurrentForRequest:_request] && NO)
    {
    ...
    {
        //// Load From file
        NSString * fileLocation = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:[NSString stringWithFormat:@"/downloads/%@", _imageUrl]];
        if ([[NSFileManager defaultManager] fileExistsAtPath:fileLocation]) {
            NSLog(@"Loaded from file");
            super.image=[UIImage imageNamed:fileLocation];
            return;
        }
        //// Still nothing so request
